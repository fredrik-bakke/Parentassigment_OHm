...
OHm <- function(inpgeno,parentfile,qc=c(geno=0.05,mind=0.10,maf=0.01,hwe=1e-6,thin=1,chrset=30),threshOMM=25,matchchecks=F,outfile){
  ...
  rm(het,less.H,high.H,hetpoor,dat); gc()
  ...
  if(matchchecks==F){
    ...
    siredam <- rbind.data.frame(data.frame(ID=sires$sireID,sex='M',stringsAsFactors=F),
                                data.frame(ID=dams$damID,sex='F',stringsAsFactors=F),stringsAsFactors=F)
    ...
    pedigreconst <- data.frame(ID=character(),sire=character(),OHsire=numeric(),
    ...
    for(i in 1:nrow(offspring)){
      ...
      if(nrow(OH.PDoffT)==0) {
        sirechosen <- NA; sirechosenOH <- NA;  
        sirepossib <- NA; sirepossibOH <- NA;
      ...
      if(nrow(OH.PDoffT)==0) {
        damchosen <- NA; damchosenOH <- NA;  
        dampossib <- NA; dampossibOH <- NA;
      } else if(nrow(OH.PDoffT)==1){
        OH.PDoffT <- OH.PDoffT[order(OH.PDoffT$OPH,decreasing=F),]
      ...
      OHmdone <- cbind.data.frame(ID=offspring[i,1],sire=sirechosen,OHsire=sirechosenOH,
      ...
      if(i==1){write.table(OHmdone,paste(outfile,'.csv',sep=''),quote=F,row.names=F,col.names=T,sep=',')
      } else {write.table(OHmdone,paste(outfile,'.csv',sep=''),quote=F,row.names=F,col.names=F,append=T,sep=',')}
    ...
    for(i in 1:nrow(matchanimsavail)){
      ...
      OHmmatchcheck <- cbind.data.frame(ID=matchanimsavail$ID[i],check=matchanimsavail$matchanims[i],
